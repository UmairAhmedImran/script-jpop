...
async def process_and_download_torrents(page, download_dir="downloads"):
    if not os.path.exists(download_dir):
    ...
    try:
        ...
        print(f"Error or timeout while waiting for the table: {e}")
        return  # Exit if there is an error or timeout
    ...
    # Loop over each row and attempt to download
    for tr in tr_elements:
        try:
            # Locate <a> tags inside the row that have "View Torrent" in their title
            view_torrent_a_tag = await tr.query_selector("a[title*='View Torrent']")
            ...
            # Only proceed if a download link is found
            if download_a_tag:
                download_link = await download_a_tag.get_attribute('href')
                print(f"Downloading: {view_torrent_text}")

                # Set up the event listener for the download
                async with page.expect_download() as download_info:
                    await download_a_tag.click()  # Simulate a click on the download link
                    download = await download_info.value  # Get the download object

                # Handle the download
                download_path = await download.path()
                if download_path:
                    new_filename = download.suggested_filename()  # Use the suggested filename
                    new_filepath = os.path.join(download_dir, new_filename)
                    # Move the downloaded file
                    os.rename(download_path, new_filepath)
                    print(f"Downloaded and saved as: {new_filepath}")
                else:
                    print(
                        f"Failed to get the download path for {view_torrent_text}")
...
async def main():
    async with async_playwright() as p:
        ...
        # Loop to process and download torrents on each page
        while True:
            await process_and_download_torrents(page)
